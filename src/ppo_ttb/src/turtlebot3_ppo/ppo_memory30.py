...
class ReplayMemory(object):
    ...
    def load(self, file_name):
        ...
        rospy.loginfo("folder %s", folder)
        # folder = folder.replace("dqn_ttb/src/turtlebot3_dqn", "dqn_ttb/nodes/")
        folder = folder.replace("ppo_ttb/src/turtlebot3_ppo", "ppo_ttb/nodes/")
        rospy.loginfo("folder %s", folder)
        load_path = folder + file_name + ".pkl"
        rospy.loginfo("load_path %s", load_path)
        # load_path = file_name + ".pkl"

        with open(load_path, "rb") as infile:
            rospy.loginfo("infile %s", infile)
